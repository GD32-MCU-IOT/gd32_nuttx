/****************************************************************************
 * boards/arm/gd32e11x/gd32e113vb-eval/src/gd32e11x_pmbuttons.c
 *
...
static int button_handler(int irq, void *context, void *arg)
{
  /* At this point the MCU should have already awakened.  The state change
   * will be handled in the IDLE loop when the system is re-awakened.  The
   * button interrupt handler should be totally ignorant of the PM activities
   * and should report button activity as if nothing special happened.
   */
 ...
 *
 * Description:
 *   Configure the TAMPER user button of the GD32E113VB-EVAL board as an EXTI
 *   wakeup source, so it is able to wake up the MCU from the low power modes
 *   (Sleep and Deep-sleep) and to report user activity to the PM subsystem.
 *
 ****************************************************************************/
