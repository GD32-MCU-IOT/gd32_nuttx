/****************************************************************************
 * boards/arm/gd32e11x/gd32e113vb-eval/src/gd32e11x_gpio.c
 *
                           ...
                           enum gpio_pintype_e gpio_pintype)
{
  ...
  if (gpio_pintype >= GPIO_NPINTYPES)
    {
  ...
  switch (gpio_pintype)
    {
    case GPIO_INPUT_PIN:
      gd32_gpio_config(GPIO_CFG_INPUT | GPIO_CFG_CTL_INFLOAT |
                       pinconfig);
      gpio_ops = &gpin_ops;
      break;

    case GPIO_INPUT_PIN_PULLUP:
      gd32_gpio_config(GPIO_CFG_INPUT | GPIO_CFG_CTL_IPU |
                       pinconfig);
      gpio_ops = &gpin_ops;
      break;

    case GPIO_INPUT_PIN_PULLDOWN:
      gd32_gpio_config(GPIO_CFG_INPUT | GPIO_CFG_CTL_IPD |
                       pinconfig);
      gpio_ops = &gpin_ops;
      break;

    case GPIO_OUTPUT_PIN:
      gd32_gpio_config(GPIO_CFG_OUTPUT | GPIO_CFG_CTL_OUTPP |
                       GPIO_CFG_SPEED_50MHZ | pinconfig);
      gpio_ops = &gpout_ops;
      break;

    case GPIO_OUTPUT_PIN_OPENDRAIN:
      gd32_gpio_config(GPIO_CFG_OUTPUT | GPIO_CFG_CTL_OUTOD |
                       GPIO_CFG_SPEED_50MHZ | pinconfig);
      gpio_ops = &gpout_ops;
      break;
...
static int gpint_attach(struct gpio_dev_s *dev, pin_interrupt_t callback)
{
  ...
  /* Get pin number from configuration */

  pin = (gd32gpint->gd32gpio.pinconfig & GPIO_CFG_PIN_MASK) >>
         GPIO_CFG_PIN_SHIFT;

  /* Initialize EXTI for this GPIO */

  ret = gd32_exti_gpioirq_init(g_gpiointinputs[gd32gpint->gd32gpio.id],
                               EXTI_INTERRUPT, EXTI_TRIG_BOTH, &gpio_irq);
  ...
  gd32_exti_gpio_irq_attach(pin, gd32gpio_interrupt,
                            &g_gpint[gd32gpint->gd32gpio.id]);

  /* Disable the interrupt initially */
...
static int gpint_enable(struct gpio_dev_s *dev, bool enable)
{
  ...
  /* Get EXTI line number */

  ret = gd32_gpio_exti_linex_get(g_gpiointinputs[gd32gpint->gd32gpio.id],
                                 &linex);
  if (ret < 0)
    {
      leave_critical_section(flags);
  ...
  if (enable)
      ...
      if (gd32gpint->callback != NULL)
        {
          ...
          gd32_exti_interrupt_enable(linex);
  ...
  else
    {
      ...
      gd32_exti_interrupt_disable(linex);
  ...
  for (i = 0; i < BOARD_NGPIOIN; i++)
    {
      ...
      if (ret < 0)
        {
          gpioerr("ERROR: gpio_pin_register failed for gpio%d: %d\n",
                  pincount, ret);
          return ret;
        }
  ...
  for (i = 0; i < BOARD_NGPIOINT; i++)
    {
      ...
      ret = gpio_pin_register(&g_gpint[i].gd32gpio.gpio, pincount);
      if (ret < 0)
        {
          gpioerr("ERROR: gpio_pin_register failed for gpio%d: %d\n",
                  pincount, ret);
