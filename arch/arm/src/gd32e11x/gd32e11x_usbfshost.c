/****************************************************************************
 * arch/arm/src/gd32e11x/gd32e11x_usbfshost.c
 *
...
#include "gd32e11x_gpio.h"   /* GPIO configuration definitions */
#include <arch/board/board.h> /* May redefine GPIO settings */
#include "gd32e11x_rcu.h"

#include "arm_internal.h"
#include "gd32e11x_usbhost.h"
 ...
 *  CONFIG_GD32E11X_USBFS_DESCSIZE - Maximum size of a descriptor.  Default:
 *    128
 ...
 *  CONFIG_GD32E11X_USBHOST_PKTDUMP - Dump all incoming and outgoing USB
...
#ifndef CONFIG_GD32E11X_USBFS_RXFIFO_SIZE
#  define CONFIG_GD32E11X_USBFS_RXFIFO_SIZE 128
#endif

/* Default host non-periodic Tx FIFO size */

#ifndef CONFIG_GD32E11X_USBFS_NPTXFIFO_SIZE
#  define CONFIG_GD32E11X_USBFS_NPTXFIFO_SIZE 96
#endif

/* Default host periodic Tx fifo size register */

#ifndef CONFIG_GD32E11X_USBFS_PTXFIFO_SIZE
#  define CONFIG_GD32E11X_USBFS_PTXFIFO_SIZE 96
#endif

/* Maximum size of a descriptor */

#ifndef CONFIG_GD32E11X_USBFS_DESCSIZE
#  define CONFIG_GD32E11X_USBFS_DESCSIZE 128
#endif

/* Register/packet debug depends on CONFIG_DEBUG */

#ifndef CONFIG_DEBUG
#  undef CONFIG_GD32E11X_USBHOST_REGDEBUG
#  undef CONFIG_GD32E11X_USBHOST_PKTDUMP
#endif
...
#define GD32_NHOST_CHANNELS      8   /* Number of host channels */
#define GD32_MAX_PACKET_SIZE     64  /* Full speed max packet size */
#define GD32_EP0_DEF_PACKET_SIZE 8   /* EP0 default packet size */
#define GD32_EP0_MAX_PACKET_SIZE 64  /* EP0 FS max packet size */
#define GD32_MAX_TX_FIFOS        8   /* Max number of TX FIFOs */
#define GD32_MAX_PKTCOUNT        256 /* Max packet count */
#define GD32_RETRY_COUNT         3   /* Number of ctrl transfer retries */

/* Delays *******************************************************************/

#define GD32_READY_DELAY         200000      /* In loop counts */
#define GD32_FLUSH_DELAY         200000      /* In loop counts */
#define GD32_SETUP_DELAY         SEC2TICK(5) /* 5 seconds in system ticks */
#define GD32_DATANAK_DELAY       SEC2TICK(5) /* 5 seconds in system ticks */
 ...
 * in the structure could be moved in struct gd32_ubhost_s to achieve
 * some memory savings.
 */
...
struct gd32_chan_s
{
  ...
  volatile uint8_t  chreason;  /* Channel halt reason. See enum gd32_chreason_e */
  uint8_t           chidx;     /* Channel index */
  uint8_t           epno;      /* Device endpoint number (0-127) */
  uint8_t           eptype;    /* See USBFS_EPTYPE_* definitions */
...
struct gd32_usbhost_s
{
  /* Common device fields.  This must be the first thing defined in the
   * structure so that it is possible to simply cast from struct usbhost_s
   * to struct gd32_usbhost_s.
   */
  ...
  struct gd32_ctrlinfo_s ep0;    /* Root hub port EP0 description */
  ...
  struct gd32_chan_s chan[GD32_MAX_TX_FIFOS];
};
...
#ifdef CONFIG_GD32E11X_USBHOST_REGDEBUG
static void gd32_printreg(uint32_t addr, uint32_t val, bool iswrite);
static void gd32_checkreg(uint32_t addr, uint32_t val, bool iswrite);
static uint32_t gd32_getreg(uint32_t addr);
static void gd32_putreg(uint32_t addr, uint32_t value);
#else
#  define gd32_getreg(addr)     getreg32(addr)
#  define gd32_putreg(addr,val) putreg32(val,addr)
#endif

static inline void gd32_modifyreg(uint32_t addr, uint32_t clrbits,
                                     uint32_t setbits);

#ifdef CONFIG_GD32E11X_USBHOST_PKTDUMP
#  define gd32_pktdump(m,b,n) lib_dumpbuffer(m,b,n)
#else
#  define gd32_pktdump(m,b,n)
#endif

/* Byte stream access helper functions **************************************/

static inline uint16_t gd32_getle16(const uint8_t *val);

/* Channel management *******************************************************/

static int gd32_chan_alloc(struct gd32_usbhost_s *priv);
static inline void gd32_chan_free(struct gd32_usbhost_s *priv,
                                     int chidx);
static inline void gd32_chan_freeall(struct gd32_usbhost_s *priv);
static void gd32_chan_configure(struct gd32_usbhost_s *priv,
                                   int chidx);
static void gd32_chan_halt(struct gd32_usbhost_s *priv,
                              int chidx, enum gd32_chreason_e chreason);
static int gd32_chan_waitsetup(struct gd32_usbhost_s *priv,
                                  struct gd32_chan_s *chan);
#ifdef CONFIG_USBHOST_ASYNCH
static int gd32_chan_asynchsetup(struct gd32_usbhost_s *priv,
                                    struct gd32_chan_s *chan,
                                    usbhost_asynch_t callback,
                                    void *arg);
#endif
static int gd32_chan_wait(struct gd32_usbhost_s *priv,
                             struct gd32_chan_s *chan);
static void gd32_chan_wakeup(struct gd32_usbhost_s *priv,
                                struct gd32_chan_s *chan);
static int gd32_ctrlchan_alloc(struct gd32_usbhost_s *priv,
                                  uint8_t epno, uint8_t funcaddr,
                                  uint8_t speed,
                                  struct gd32_ctrlinfo_s *ctrlep);
static int gd32_ctrlep_alloc(struct gd32_usbhost_s *priv,
                                const struct usbhost_epdesc_s *epdesc,
                                usbhost_ep_t *ep);
static int gd32_xfrep_alloc(struct gd32_usbhost_s *priv,
                                const struct usbhost_epdesc_s *epdesc,
                                usbhost_ep_t *ep);

/* Control/data transfer logic **********************************************/

static void gd32_transfer_start(struct gd32_usbhost_s *priv,
                                   int chidx);
static int gd32_ctrl_sendsetup(struct gd32_usbhost_s *priv,
                                  struct gd32_ctrlinfo_s *ep0,
                                  const struct usb_ctrlreq_s *req);
static int gd32_ctrl_senddata(struct gd32_usbhost_s *priv,
                                 struct gd32_ctrlinfo_s *ep0,
                                 uint8_t *buffer, unsigned int buflen);
static int gd32_ctrl_recvdata(struct gd32_usbhost_s *priv,
                                 struct gd32_ctrlinfo_s *ep0,
                                 uint8_t *buffer, unsigned int buflen);
static int gd32_in_setup(struct gd32_usbhost_s *priv, int chidx);
static ssize_t gd32_in_transfer(struct gd32_usbhost_s *priv,
                                   int chidx, uint8_t *buffer,
                                   size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static void gd32_in_next(struct gd32_usbhost_s *priv,
                            struct gd32_chan_s *chan);
static int gd32_in_asynch(struct gd32_usbhost_s *priv, int chidx,
                             uint8_t *buffer, size_t buflen,
                             usbhost_asynch_t callback, void *arg);
#endif
static int gd32_out_setup(struct gd32_usbhost_s *priv, int chidx);
static ssize_t gd32_out_transfer(struct gd32_usbhost_s *priv,
                                    int chidx, uint8_t *buffer,
                                    size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static void gd32_out_next(struct gd32_usbhost_s *priv,
                             struct gd32_chan_s *chan);
static int gd32_out_asynch(struct gd32_usbhost_s *priv, int chidx,
                              uint8_t *buffer, size_t buflen,
                              usbhost_asynch_t callback, void *arg);
#endif
...
static void gd32_gint_wrpacket(struct gd32_usbhost_s *priv,
                                  uint8_t *buffer, int chidx,
                                  int buflen);
static inline void gd32_gint_hcinisr(struct gd32_usbhost_s *priv,
                                        int chidx);
static inline void gd32_gint_hcoutisr(struct gd32_usbhost_s *priv,
                                         int chidx);
static void gd32_gint_connected(struct gd32_usbhost_s *priv);
static void gd32_gint_disconnected(struct gd32_usbhost_s *priv);

/* Second level interrupt handlers */

#ifdef CONFIG_GD32E11X_USBFS_SOFINTR
static inline void gd32_gint_sofisr(struct gd32_usbhost_s *priv);
#endif
static inline void
gd32_gint_rxflvlisr(struct gd32_usbhost_s *priv);
static inline void
gd32_gint_nptxfeisr(struct gd32_usbhost_s *priv);
static inline void gd32_gint_ptxfeisr(struct gd32_usbhost_s *priv);
static inline void gd32_gint_hcisr(struct gd32_usbhost_s *priv);
static inline void gd32_gint_hprtisr(struct gd32_usbhost_s *priv);
static inline void gd32_gint_discisr(struct gd32_usbhost_s *priv);
static inline void gd32_gint_ipxfrisr(struct gd32_usbhost_s *priv);

/* First level, global interrupt handler */

static int gd32_gint_isr(int irq, void *context, void *arg);

/* Interrupt controls */

static void gd32_gint_enable(void);
static void gd32_gint_disable(void);
static inline void gd32_hostinit_enable(void);
static void gd32_txfe_enable(struct gd32_usbhost_s *priv,
                                int chidx);

/* USB host controller operations *******************************************/

static int gd32_wait(struct usbhost_connection_s *conn,
                        struct usbhost_hubport_s **hport);
static int gd32_rh_enumerate(struct gd32_usbhost_s *priv,
                                struct usbhost_connection_s *conn,
                                struct usbhost_hubport_s *hport);
static int gd32_enumerate(struct usbhost_connection_s *conn,
                             struct usbhost_hubport_s *hport);

static int gd32_ep0configure(struct usbhost_driver_s *drvr,
                                usbhost_ep_t ep0, uint8_t funcaddr,
                                uint8_t speed, uint16_t maxpacketsize);
static int gd32_epalloc(struct usbhost_driver_s *drvr,
                           const struct usbhost_epdesc_s *epdesc,
                           usbhost_ep_t *ep);
static int gd32_epfree(struct usbhost_driver_s *drvr,
                          usbhost_ep_t ep);
static int gd32_alloc(struct usbhost_driver_s *drvr,
                         uint8_t **buffer, size_t *maxlen);
static int gd32_free(struct usbhost_driver_s *drvr,
                        uint8_t *buffer);
static int gd32_ioalloc(struct usbhost_driver_s *drvr,
                           uint8_t **buffer, size_t buflen);
static int gd32_iofree(struct usbhost_driver_s *drvr,
                          uint8_t *buffer);
static int gd32_ctrlin(struct usbhost_driver_s *drvr,
                          usbhost_ep_t ep0,
                          const struct usb_ctrlreq_s *req,
                          uint8_t *buffer);
static int gd32_ctrlout(struct usbhost_driver_s *drvr,
                           usbhost_ep_t ep0,
                           const struct usb_ctrlreq_s *req,
                           const uint8_t *buffer);
static ssize_t gd32_transfer(struct usbhost_driver_s *drvr,
                                usbhost_ep_t ep, uint8_t *buffer,
                                size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static int gd32_asynch(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                          uint8_t *buffer, size_t buflen,
                          usbhost_asynch_t callback, void *arg);
#endif
static int gd32_cancel(struct usbhost_driver_s *drvr,
                          usbhost_ep_t ep);
#ifdef CONFIG_USBHOST_HUB
static int gd32_connect(struct usbhost_driver_s *drvr,
                           struct usbhost_hubport_s *hport,
                           bool connected);
#endif
static void gd32_disconnect(struct usbhost_driver_s *drvr,
                               struct usbhost_hubport_s *hport);

/* Initialization ***********************************************************/

static void gd32_portreset(struct gd32_usbhost_s *priv);
static void gd32_flush_txfifos(uint32_t txfnum);
static void gd32_flush_rxfifo(void);
static void gd32_vbusdrive(struct gd32_usbhost_s *priv,
                              bool state);
static void gd32_host_initialize(struct gd32_usbhost_s *priv);

static inline void gd32_sw_initialize(struct gd32_usbhost_s *priv);
static inline int gd32_hw_initialize(struct gd32_usbhost_s *priv);
...
static struct gd32_usbhost_s g_usbhost =
{
 ...
 *
 * Description:
 *   Print the contents of an GD32E11x register operation
 *
 ****************************************************************************/
...
static void gd32_checkreg(uint32_t addr, uint32_t val, bool iswrite)
{
  ...
  if (addr == prevaddr && val == preval && prevwrite == iswrite)
  ...
  else
    {
          ...
          if (count == 1)
            {
              ...
              gd32_printreg(prevaddr, preval, prevwrite);
            }
          else
            {
              /* No.. More than one. */
      ...
      gd32_printreg(addr, val, iswrite);
    }
}
#endif
...
#ifdef CONFIG_GD32E11X_USBHOST_REGDEBUG
static void gd32_putreg(uint32_t addr, uint32_t val)
{
  /* Check if we need to print this value */

  gd32_checkreg(addr, val, true);
 ...
 *
 * Description:
 *   Modify selected bits of a GD32E11x register.
 *
 ****************************************************************************/
...
static inline void gd32_modifyreg(uint32_t addr, uint32_t clrbits,
                                     uint32_t setbits)
{
  gd32_putreg(addr, (((gd32_getreg(addr)) & ~clrbits) | setbits));
}
...
static int gd32_chan_alloc(struct gd32_usbhost_s *priv)
{
  ...
  for (chidx = 0; chidx < GD32_NHOST_CHANNELS; chidx++)
    {
...
static void gd32_chan_free(struct gd32_usbhost_s *priv, int chidx)
{
  DEBUGASSERT((unsigned)chidx < GD32_NHOST_CHANNELS);

  /* Halt the channel */

  gd32_chan_halt(priv, chidx, CHREASON_FREED);
...
static inline void gd32_chan_freeall(struct gd32_usbhost_s *priv)
{
  ...
  for (chidx = 2; chidx < GD32_NHOST_CHANNELS; chidx++)
    {
      gd32_chan_free(priv, chidx);
    }
}

/****************************************************************************
 * Name: gd32_chan_configure
 *
 * Description:
...
static void gd32_chan_configure(struct gd32_usbhost_s *priv,
                                   int chidx)
{
  struct gd32_chan_s *chan = &priv->chan[chidx];
  uint32_t regval;

  /* Clear any old pending interrupts for this host channel. */

  gd32_putreg(GD32_USBFS_HCHINTF(chidx), 0xffffffff);
  ...
  switch (chan->eptype)
    {
    case USBFS_EPTYPE_CTRL:
    case USBFS_EPTYPE_BULK:
      {
        ...
        if (chan->eptype == USBFS_EPTYPE_CTRL)
          {
            intrace  = USBFS_VTRACE2_CHANCONF_CTRL_IN;
            outtrace = USBFS_VTRACE2_CHANCONF_CTRL_OUT;
          }
        else
          {
            intrace  = USBFS_VTRACE2_CHANCONF_BULK_IN;
            outtrace = USBFS_VTRACE2_CHANCONF_BULK_OUT;
          }
#endif

        /* Interrupts required for CTRL and BULK endpoints */

        regval |= (USBFS_HCHINTF_TF | USBFS_HCHINTF_STALL |
             USBFS_HCHINTF_NAK | USBFS_HCHINTF_USBER |
             USBFS_HCHINTF_DTER);
    ...
    case USBFS_EPTYPE_INTR:
      {
        /* Interrupts required for INTR endpoints */

        regval |= (USBFS_HCHINTF_TF  | USBFS_HCHINTF_STALL |
                   USBFS_HCHINTF_NAK   | USBFS_HCHINTF_USBER |
                   USBFS_HCHINTF_REQOVR | USBFS_HCHINTF_DTER);

        /* Additional setting for IN endpoints */

        if (chan->in)
          {
            usbhost_vtrace2(USBFS_VTRACE2_CHANCONF_INTR_IN, chidx,
                            chan->epno);
            regval |= USBFS_HCHINTF_BBER;
          }
#ifdef HAVE_USBHOST_TRACE_VERBOSE
          ...
          {
            usbhost_vtrace2(USBFS_VTRACE2_CHANCONF_INTR_OUT, chidx,
                            chan->epno);
          }
#endif
      }
    ...
    case USBFS_EPTYPE_ISOC:
      {
        /* Interrupts required for ISOC endpoints */

        regval |= (USBFS_HCHINTF_TF | USBFS_HCHINTF_ACK |
             USBFS_HCHINTF_REQOVR);

        /* Additional setting for IN endpoints */

        if (chan->in)
          {
            usbhost_vtrace2(USBFS_VTRACE2_CHANCONF_ISOC_IN, chidx,
                            chan->epno);
            regval |= (USBFS_HCHINTF_USBER | USBFS_HCHINTF_BBER);
          }
#ifdef HAVE_USBHOST_TRACE_VERBOSE
        else
          {
            usbhost_vtrace2(USBFS_VTRACE2_CHANCONF_ISOC_OUT, chidx,
                            chan->epno);
          }
#endif
      }
  ...
  gd32_putreg(GD32_USBFS_HCHINTEN(chidx), regval);

  /* Enable the top level host channel interrupt. */

  gd32_modifyreg(GD32_USBFS_HACHINTEN, 0, USBFS_HACHINT(chidx));

  /* Make sure host channel interrupts are enabled. */

  gd32_modifyreg(GD32_USBFS_GINTEN, 0, USBFS_GINTF_HCIF);

  /* Program the HCCHAR register */

  regval = ((uint32_t)chan->maxpacket << USBFS_HCHCTL_MPL_SHIFT) |
           ((uint32_t)chan->epno      << USBFS_HCHCTL_EPNUM_SHIFT) |
           ((uint32_t)chan->eptype    << USBFS_HCHCTL_EPTYPE_SHIFT) |
           ((uint32_t)chan->funcaddr  << USBFS_HCHCTL_DAR_SHIFT);

  /* Special case settings for low speed devices */

  if (chan->speed == USB_SPEED_LOW)
    {
      regval |= USBFS_HCHCTL_LSD;
    }
  ...
  if (chan->in)
    {
      regval |= USBFS_HCHCTL_EPDIR_IN;
    }

  /* Special case settings for INTR endpoints */

  if (chan->eptype == USBFS_EPTYPE_INTR)
    {
      regval |= USBFS_HCHCTL_ODDFRM;
    }

  /* Write the channel configuration */

  gd32_putreg(GD32_USBFS_HCHCTL(chidx), regval);
}
...
static void gd32_chan_halt(struct gd32_usbhost_s *priv, int chidx,
                              enum gd32_chreason_e chreason)
{
  uint32_t hcchar;
  uint32_t intmsk;
  uint32_t eptype;
  ...
  usbhost_vtrace2(USBFS_VTRACE2_CHANHALT, chidx, chreason);

  priv->chan[chidx].chreason = (uint8_t)chreason;

  /* "The application can disable any channel by programming the
   *  USBFS_HCHxCTL register with the CHDIS and CHENA bits set to 1. This
   *  enables the USBFS host to flush the posted requests (if any) and
   *  generates a channel halted interrupt. The application must wait for
   *  the CHH interrupt in USBFS_HCHxINTF before reallocating the channel for
   *  other transactions.  The USBFS host does not interrupt the
   *  transaction that has already been started on the USB."
   */

  hcchar  = gd32_getreg(GD32_USBFS_HCHCTL(chidx));
  hcchar |= (USBFS_HCHCTL_CDIS | USBFS_HCHCTL_CEN);

  /* Get the endpoint type from the HCCHAR register */

  eptype = hcchar & USBFS_HCHCTL_EPTYPE_MASK;
   ...
   *  the channel), by programming the USBFS_HCHxCTL register with the
   *  CHDIS bit set to 1, and the CHENA bit cleared to 0.
   */

  if (eptype == USBFS_HCHCTL_EPTYPE_CTRL ||
      eptype == USBFS_HCHCTL_EPTYPE_BULK)
    {
      /* Get the number of words available in the non-periodic Tx FIFO. */

      avail = gd32_getreg(GD32_USBFS_HNPTFQSTAT) &
              USBFS_HNPTFQSTAT_NPTXFS_MASK;
    }
  else
    {
      /* Get the number of words available in the non-periodic Tx FIFO. */

      avail = gd32_getreg(GD32_USBFS_HPTFQSTAT) &
              USBFS_HPTFQSTAT_PTXFS_MASK;
    }
  ...
  if (avail == 0)
    {
      ...
      hcchar &= ~USBFS_HCHCTL_CEN;
    }

  /* Unmask the CHannel Halted (CHH) interrupt */

  intmsk  = gd32_getreg(GD32_USBFS_HCHINTEN(chidx));
  intmsk |= USBFS_HCHINTF_CH;
  gd32_putreg(GD32_USBFS_HCHINTEN(chidx), intmsk);

  /* Halt the channel by setting CHDIS (and maybe CHENA) in the HCCHAR */

  gd32_putreg(GD32_USBFS_HCHCTL(chidx), hcchar);
}

/****************************************************************************
 * Name: gd32_chan_waitsetup
 *
 * Description:
                             ...
                             struct gd32_chan_s *chan)
{
   ...
   * On the USBFS global interrupt needs to be disabled.  However, here we
  ...
  if (chan->result != EBUSY)
    {
      ...
      if (chan->waiter)
        {
          ...
          usbhost_vtrace2(chan->in ? USBFS_VTRACE2_CHANWAKEUP_IN :
                                     USBFS_VTRACE2_CHANWAKEUP_OUT,
       ...
       */
      ...
      else if (chan->callback)
        {
          ...
          if (chan->in)
            {
              gd32_in_next(priv, chan);
            }
          else
            {
              gd32_out_next(priv, chan);
            }
        }
#endif
    }
                                  ...
                                  struct gd32_ctrlinfo_s *ctrlep)
{
  ...
  chan->epno      = epno;
  chan->in        = false;
  chan->eptype    = USBFS_EPTYPE_CTRL;
  chan->funcaddr  = funcaddr;
  chan->speed     = speed;
  chan->interval  = 0;
  chan->maxpacket = GD32_EP0_DEF_PACKET_SIZE;
  chan->indata1   = false;
  chan->outdata1  = false;

  /* Configure control OUT channels */

  gd32_chan_configure(priv, outndx);

  /* Allocate and initialize the control IN channel */

  inndx = gd32_chan_alloc(priv);
  if (inndx < 0)
    {
      gd32_chan_free(priv, outndx);
      return -ENOMEM;
    }
  ...
  gd32_chan_configure(priv, inndx);
  return OK;
}

/****************************************************************************
 * Name: gd32_ctrlep_alloc
 *
 * Description:
                                ...
                                usbhost_ep_t *ep)
{
  ...
  ctrlep = (struct gd32_ctrlinfo_s *)
    kmm_malloc(sizeof(struct gd32_ctrlinfo_s));
  if (ctrlep == NULL)
    {
  ...
  ret = gd32_ctrlchan_alloc(priv, epdesc->addr & USB_EPNO_MASK,
                             hport->funcaddr, hport->speed, ctrlep);
  if (ret < 0)
    {
      uerr("ERROR: gd32_ctrlchan_alloc failed: %d\n", ret);
      kmm_free(ctrlep);
      return ret;
    }
                               ...
                               usbhost_ep_t *ep)
{
  ...
  gd32_chan_configure(priv, chidx);
                                   ...
                                   int chidx)
{
  ...
  usbhost_vtrace2(USBFS_VTRACE2_STARTTRANSFER, chidx, chan->buflen);
  ...
  if (chan->buflen > maxpacket)
    {
      ...
      if (npackets > GD32_MAX_PKTCOUNT)
        {
          npackets = GD32_MAX_PKTCOUNT;
          chan->buflen = GD32_MAX_PKTCOUNT * maxpacket;
          usbhost_trace2(USBFS_TRACE2_CLIP, chidx, chan->buflen);
        }
    }
  else
    {
  ...
  regval = ((uint32_t)chan->buflen << USBFS_HCHLEN_TLEN_SHIFT) |
           ((uint32_t)npackets << USBFS_HCHLEN_PCNT_SHIFT) |
           ((uint32_t)chan->pid << USBFS_HCHLEN_DPID_SHIFT);
  gd32_putreg(GD32_USBFS_HCHLEN(chidx), regval);

  /* Setup the HCCHAR register: Frame oddness and host channel enable */

  regval = gd32_getreg(GD32_USBFS_HCHCTL(chidx));
  ...
  if ((gd32_getreg(GD32_USBFS_HFINFR) & 1) == 0)
    {
      regval |= USBFS_HCHCTL_ODDFRM;
    }
  else
  ...
  regval &= ~USBFS_HCHCTL_CDIS;
  regval |= USBFS_HCHCTL_CEN;
  gd32_putreg(GD32_USBFS_HCHCTL(chidx), regval);
  ...
  if (!chan->in && chan->buflen > 0)
    {
      ...
      switch (chan->eptype)
        {
        case USBFS_EPTYPE_CTRL: /* Non periodic transfer */
        case USBFS_EPTYPE_BULK:
          {
            /* Read the Non-periodic Tx FIFO status register */

            regval = gd32_getreg(GD32_USBFS_HNPTFQSTAT);
            avail  = ((regval & USBFS_HNPTFQSTAT_NPTXFS_MASK) >>
                       USBFS_HNPTFQSTAT_NPTXFS_SHIFT) << 2;
          }
          break;
        ...
        case USBFS_EPTYPE_ISOC:
          {
            ...
            regval = gd32_getreg(GD32_USBFS_HPTFQSTAT);
            avail  = ((regval & USBFS_HPTFQSTAT_PTXFS_MASK) >>
                       USBFS_HPTFQSTAT_PTXFS_SHIFT) << 2;
          }
          break;
      ...
      if (minsize <= avail)
        {
          ...
          gd32_gint_wrpacket(priv, chan->buffer, chidx, wrsize);
        }
      ...
      if (chan->buflen > avail)
        {
           ...
           */
           ...
           gd32_txfe_enable(priv, chidx);
        }
    }
}

/****************************************************************************
 * Name: gd32_ctrl_sendsetup
 *
 * Description:
                                  ...
                                  const struct usb_ctrlreq_s *req)
{
  ...
  do
    {
      ...
      ret = gd32_chan_waitsetup(priv, chan);
      if (ret < 0)
        {
          usbhost_trace1(USBFS_TRACE1_DEVDISCONN, 0);
          return ret;
        }

      /* Start the transfer */

      gd32_transfer_start(priv, ep0->outndx);

      /* Wait for the transfer to complete */

      ret = gd32_chan_wait(priv, chan);
      ...
      if (ret != -EAGAIN)
        {
          ...
          if (ret < 0)
            {
              usbhost_trace1(USBFS_TRACE1_TRNSFRFAILED, ret);
            }
  ...
  while (elapsed < GD32_SETUP_DELAY);

  return -ETIMEDOUT;
}

/****************************************************************************
 * Name: gd32_ctrl_senddata
 *
 * Description:
...
static int gd32_ctrl_senddata(struct gd32_usbhost_s *priv,
                                 ...
                                 uint8_t *buffer, unsigned int buflen)
{
  struct gd32_chan_s *chan = &priv->chan[ep0->outndx];
  int ret;
  ...
  chan->pid = chan->outdata1 ? USBFS_PID_DATA1 : USBFS_PID_DATA0;
  ...
  return gd32_chan_wait(priv, chan);
}

/****************************************************************************
 * Name: gd32_ctrl_recvdata
 *
 * Description:
...
static int gd32_ctrl_recvdata(struct gd32_usbhost_s *priv,
                                 ...
                                 uint8_t *buffer, unsigned int buflen)
{
  struct gd32_chan_s *chan = &priv->chan[ep0->inndx];
  int ret;

  /* Save buffer information */

  chan->pid    = USBFS_PID_DATA1;
  chan->buffer = buffer;
  chan->buflen = buflen;
  chan->xfrd   = 0;
  ...
  gd32_transfer_start(priv, ep0->inndx);
...
static int gd32_in_setup(struct gd32_usbhost_s *priv, int chidx)
{
  ...
  switch (chan->eptype)
    {
    default:
    case USBFS_EPTYPE_CTRL: /* Control */
      {
    ...
    case USBFS_EPTYPE_ISOC: /* Isochronous */
      {
        /* Set up the IN data PID */

        usbhost_vtrace2(USBFS_VTRACE2_ISOCIN, chidx, chan->buflen);
        chan->pid = USBFS_PID_DATA0;
      }
      break;

    case USBFS_EPTYPE_BULK: /* Bulk */
      {
        /* Setup the IN data PID */

        usbhost_vtrace2(USBFS_VTRACE2_BULKIN, chidx, chan->buflen);
        chan->pid = chan->indata1 ? USBFS_PID_DATA1 : USBFS_PID_DATA0;
      }
      break;

    case USBFS_EPTYPE_INTR: /* Interrupt */
      {
        /* Setup the IN data PID */

        usbhost_vtrace2(USBFS_VTRACE2_INTRIN, chidx, chan->buflen);
        chan->pid = chan->indata1 ? USBFS_PID_DATA1 : USBFS_PID_DATA0;
      }
      break;
    }
  ...
  gd32_transfer_start(priv, chidx);
  return OK;
}

/****************************************************************************
 * Name: gd32_in_transfer
 *
 * Description:
  ...
  while (chan->xfrd < chan->buflen)
    {
      ...
      ret = gd32_in_setup(priv, chidx);
      if (ret < 0)
        {
          uerr("ERROR: gd32_in_setup failed: %d\n", ret);
          return (ssize_t)ret;
        }
      ...
      if (ret < 0)
        {
          ...
          if (ret == -EAGAIN)
              ...
              if (xfrd > 0)
                {
                   ...
                   *
                   * This behavior is clearly correct for CDC/ACM
              ...
              else
                {
                  ...
                  if (chan->eptype == USBFS_EPTYPE_INTR)
                  ...
                  else
                    {
                       ...
                       *
                       * For bulk endpoints this 1 msec delay is
                   ...
                   *
                   * So which is better?  To ignore tiny delays and
          ...
          else
            {
              ...
              uerr("ERROR: gd32_chan_wait failed: %d\n", ret);
              return (ssize_t)ret;
            }
        }
      else
  ...
  if (chan->xfrd < chan->buflen && result == OK)
    {
      ...
      ret = gd32_in_setup(priv, chan->chidx);
      if (ret >= 0)
        {
          return;
        }
                             ...
                             usbhost_asynch_t callback, void *arg)
{
  ...
  ret = gd32_chan_asynchsetup(priv, chan, callback, arg);
  if (ret < 0)
    {
      uerr("ERROR: gd32_chan_asynchsetup failed: %d\n", ret);
      return ret;
    }
    ...
    case USBFS_EPTYPE_ISOC: /* Isochronous */
      {
        ...
        usbhost_vtrace2(USBFS_VTRACE2_ISOCOUT, chidx, chan->buflen);
        chan->pid = USBFS_PID_DATA0;
      }
      break;
    ...
    case USBFS_EPTYPE_BULK: /* Bulk */
      {
        ...
        usbhost_vtrace2(USBFS_VTRACE2_BULKOUT, chidx, chan->buflen);
        chan->pid = chan->outdata1 ? USBFS_PID_DATA1 : USBFS_PID_DATA0;
      }
      break;
    ...
    case USBFS_EPTYPE_INTR: /* Interrupt */
      {
        ...
        usbhost_vtrace2(USBFS_VTRACE2_INTROUT, chidx, chan->buflen);
        chan->pid = chan->outdata1 ? USBFS_PID_DATA1 : USBFS_PID_DATA0;
  ...
  while (buflen > 0 || zlp)
    {
      ...
      ret = gd32_out_setup(priv, chidx);
      if (ret < 0)
        {
          uerr("ERROR: gd32_out_setup failed: %d\n", ret);
          return (ssize_t)ret;
        }
      ...
      if (ret < 0)
        {
          ...
          /* Break out unless this is still just a NAK with no transferred
           * data and the retry timeout has not elapsed.
           */

          if (ret != -EAGAIN || elapsed >= GD32_DATANAK_DELAY ||
              chan->xfrd > 0)
            {
              /* Break out and return the error */
          ...
          gd32_flush_txfifos(USBFS_GRSTCTL_TXFNUM_HALL);
  ...
  if (chan->xfrd < chan->buflen && result == OK)
    {
      ...
      ret = gd32_out_setup(priv, chan->chidx);
      if (ret >= 0)
        {
          return;
        }
                                  ...
                                  uint8_t *buffer, int chidx, int buflen)
{
  ...
  gd32_pktdump("Sending", buffer, buflen);
  ...
  fifo = GD32_USBFS_DFIFO_HCH(chidx);
...
/****************************************************************************
 * Name: gd32_gint_hcinisr
 *
 * Description:
 *   USB USBFS host IN channels interrupt handler
 *
                                        ...
                                        int chidx)
{
  ...
  pending = gd32_getreg(GD32_USBFS_HCHINTF(chidx));
  regval  = gd32_getreg(GD32_USBFS_HCHINTEN(chidx));
  ...
  else if ((pending & USBFS_HCHINTF_TF) != 0)
    {
      ...
      if (chan->eptype == USBFS_EPTYPE_CTRL ||
          chan->eptype == USBFS_EPTYPE_BULK)
        {
          ...
          gd32_putreg(GD32_USBFS_HCHINTF(chidx), USBFS_HCHINTF_NAK);
        }
      else if (chan->eptype == USBFS_EPTYPE_INTR)
        {
  ...
  else if ((pending & USBFS_HCHINTF_CH) != 0)
    {
      /* Mask the CHannel Halted (CHH) interrupt */

      regval  = gd32_getreg(GD32_USBFS_HCHINTEN(chidx));
      regval &= ~USBFS_HCHINTF_CH;
      gd32_putreg(GD32_USBFS_HCHINTEN(chidx), regval);
      ...
      gd32_putreg(GD32_USBFS_HCHINTF(chidx), USBFS_HCHINTF_CH);
    }

  /* Check for a pending Transaction ERror (TXERR) interrupt */

  else if ((pending & USBFS_HCHINTF_USBER) != 0)
    {
      /* Halt the channel when a STALL, TXERR, BBERR or DTERR interrupt is
       * received on the channel.
       */

      gd32_chan_halt(priv, chidx, CHREASON_TXERR);

      /* Clear the Transaction ERror (TXERR) condition */

      gd32_putreg(GD32_USBFS_HCHINTF(chidx), USBFS_HCHINTF_USBER);
    }

  /* Check for a pending NAK response received (NAK) interrupt */

  else if ((pending & USBFS_HCHINTF_NAK) != 0)
    {
      ...
      if (chan->eptype == USBFS_EPTYPE_INTR ||
          chan->eptype == USBFS_EPTYPE_BULK)
        {
          ...
          gd32_chan_halt(priv, chidx, CHREASON_NAK);
        }

      /* Re-activate CTRL and BULK channels.
       *
       * This can cause a lot of interrupts!
       * Bulk endpoints not re-activated.
       */

      else if (chan->eptype == USBFS_EPTYPE_CTRL)
        {
          /* Re-activate the channel by clearing CHDIS and assuring that
           * CHENA is set
           *
           * Set channel reason to NACK?
           */
  ...
  gd32_chan_wakeup(priv, chan);
}

/****************************************************************************
 * Name: gd32_gint_hcoutisr
 *
 * Description:
 *   USB USBFS host OUT channels interrupt handler
 *
 *   One the completion of the transfer, the channel result byte may be set
 *   as follows:
 *
                                         ...
                                         int chidx)
{
  ...
  else if ((pending & USBFS_HCHINTF_REQOVR) != 0)
    {
  ...
  /* Check for a pending STALL response receive (STALL) interrupt */
  ...
  else if ((pending & USBFS_HCHINTF_STALL) != 0)
    {
      ...
      gd32_putreg(GD32_USBFS_HCHINTF(chidx), USBFS_HCHINTF_STALL);
  ...
  else if (pending & USBFS_HCHINTF_DTER)
    {
      /* Halt the channel when a STALL, TXERR, BBERR or DTERR interrupt is
       * received on the channel.
       */
      ...
      gd32_putreg(GD32_USBFS_HCHINTF(chidx),
                     (USBFS_HCHINTF_DTER | USBFS_HCHINTF_NAK));
    }
      ...
      if (chan->chreason == CHREASON_XFRC)
        {
          ...
          if ((regval & USBFS_HCHCTL_EPTYPE_MASK) ==
              USBFS_HCHCTL_EPTYPE_BULK &&
              (chan->npackets & 1) != 0)
            {
  ...
  if (!priv->connected)
    {
      ...
      usbhost_vtrace1(USBFS_VTRACE1_CONNECTED, 0);
      priv->connected = true;
      priv->change    = true;
  ...
  if (priv->connected)
    {
      ...
      usbhost_vtrace1(USBFS_VTRACE1_DISCONNECTED, 0);
      ...
      /* Free the EP0 control channels (indices 0 and 1) which are
       * skipped by gd32_chan_freeall().
       */

      gd32_chan_free(priv, priv->ep0.outndx);
      gd32_chan_free(priv, priv->ep0.inndx);
      gd32_chan_freeall(priv);
...
/****************************************************************************
 * Name: gd32_gint_sofisr
 *
 * Description:
 *   USBFS start-of-frame interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_sofisr(struct gd32_usbhost_s *priv)
{
  ...
  gd32_putreg(GD32_USBFS_GINTF, USBFS_GINTF_SOF);
}
#endif

/****************************************************************************
 * Name: gd32_gint_rxflvlisr
 *
 * Description:
 *   USBFS RxFIFO non-empty interrupt handler
 *
 ****************************************************************************/

static inline void gd32_gint_rxflvlisr(struct gd32_usbhost_s *priv)
{
  uint32_t *dest;
  uint32_t grxsts;
  uint32_t intmsk;
  ...
  intmsk  = gd32_getreg(GD32_USBFS_GINTEN);
  intmsk &= ~USBFS_GINTF_RXFNEIF;
  gd32_putreg(GD32_USBFS_GINTEN, intmsk);

  /* Read and pop the next status from the Rx FIFO */

  grxsts = gd32_getreg(GD32_USBFS_GRSTATP);
  uinfo("GRXSTS: %08" PRIx32 "\n", grxsts);

  /* Isolate the channel number/index in the status word */

  chidx = (grxsts & USBFS_GRXSTAH_CNUM_MASK) >> USBFS_GRXSTAH_CNUM_SHIFT;
  ...
  switch (grxsts & USBFS_GRXSTAH_RPCKST_MASK)
    {
    case USBFS_GRXSTAH_RPCKST_INRECVD: /* IN data packet received */
      {
        /* Read the data into the host buffer. */

        bcnt = (grxsts & USBFS_GRXSTAH_BCOUNT_MASK) >>
               USBFS_GRXSTAH_BCOUNT_SHIFT;
        if (bcnt > 0 && priv->chan[chidx].buffer != NULL)
          {
            /* Verify that the received data fits in the user buffer */

            if (priv->chan[chidx].xfrd + bcnt > priv->chan[chidx].buflen)
              {
                bcnt = priv->chan[chidx].buflen - priv->chan[chidx].xfrd;
              }

            /* Transfer the packet from the Rx FIFO into the user buffer */

            dest   = (uint32_t *)priv->chan[chidx].buffer;
            fifo   = GD32_USBFS_DFIFO_HCH(0);
            bcnt32 = (bcnt + 3) >> 2;

            for (i = 0; i < bcnt32; i++)
              {
                *dest++ = gd32_getreg(fifo);
              }

            gd32_pktdump("Received", priv->chan[chidx].buffer, bcnt);
            ...
            hctsiz = gd32_getreg(GD32_USBFS_HCHLEN(chidx));
            if ((hctsiz & USBFS_HCHLEN_PCNT_MASK) != 0)
              {
                /* Re-activate the channel when more packets are expected */

                hcchar |= USBFS_HCHCTL_CEN;
                hcchar &= ~USBFS_HCHCTL_CDIS;
                gd32_putreg(GD32_USBFS_HCHCTL(chidx), hcchar);
              }
          }
      }
    ...
    case USBFS_GRXSTAH_RPCKST_INDONE:  /* IN transfer completed */
    case USBFS_GRXSTAH_RPCKST_DTOGERR: /* Data toggle error */
    case USBFS_GRXSTAH_RPCKST_HALTED:  /* Channel halted */
    default:
      break;
    }
...
/****************************************************************************
 * Name: gd32_gint_nptxfeisr
 *
 * Description:
 *   USBFS non-periodic TxFIFO empty interrupt handler
 *
 ****************************************************************************/

static inline void gd32_gint_nptxfeisr(struct gd32_usbhost_s *priv)
{
  struct gd32_chan_s *chan;
  uint32_t     regval;
  unsigned int wrsize;
  ...
  if (chan->xfrd >= chan->buflen)
    {
      ...
      gd32_modifyreg(GD32_USBFS_GINTEN, USBFS_GINTF_NPTXFEIF, 0);
      return;
    }
...
/****************************************************************************
 * Name: gd32_gint_ptxfeisr
 *
 * Description:
 *   USBFS periodic TxFIFO empty interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_ptxfeisr(struct gd32_usbhost_s *priv)
{
  ...
  if (chan->xfrd >= chan->buflen)
    {
      ...
      gd32_modifyreg(GD32_USBFS_GINTEN, USBFS_GINTF_PTXFEIF, 0);
      return;
    }
  ...
  /* Write the next group of packets into the Tx FIFO */
    ...
    uinfo("HPTXSTS: %08" PRIx32 " chidx: %d avail: %d buflen: %d "
      "xfrd: %d wrsize: %d\n",
      regval, chidx, avail, chan->buflen, chan->xfrd, wrsize);
...
/****************************************************************************
 * Name: gd32_gint_hcisr
 *
 * Description:
 *   USB USBFS host channels interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_hcisr(struct gd32_usbhost_s *priv)
{
   ...
   * register. Each bit i, i=0...(GD32_NHOST_CHANNELS-1), corresponds to
   * a pending interrupt on channel i.
   */

  haint = gd32_getreg(GD32_USBFS_HACHINT);
  for (i = 0; i < GD32_NHOST_CHANNELS; i++)
    {
      /* Is an interrupt pending on this channel? */

      if ((haint & USBFS_HACHINT(i)) != 0)
        {
          /* Yes... read the HCCHAR register to get the direction bit */

          hcchar = gd32_getreg(GD32_USBFS_HCHCTL(i));

          /* Was this an interrupt on an IN or an OUT channel? */

          if ((hcchar & USBFS_HCHCTL_EPDIR) != 0)
            {
              /* Handle the HC IN channel interrupt */

              gd32_gint_hcinisr(priv, i);
            }
          else
            {
              /* Handle the HC OUT channel interrupt */

              gd32_gint_hcoutisr(priv, i);
            }
        }
    }
}
...
/****************************************************************************
 * Name: gd32_gint_hprtisr
 *
 * Description:
 *   USB USBFS host port interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_hprtisr(struct gd32_usbhost_s *priv)
{
  ...
  usbhost_vtrace1(USBFS_VTRACE1_GINT_HPRT, 0);

  /* Read the port status and control register (HPRT) */

  hprt = gd32_getreg(GD32_USBFS_HPCS);
      ...
      if ((hprt & USBFS_HPCS_PE) != 0)
        {
          ...
          hcfg = gd32_getreg(GD32_USBFS_HCTL);

          /* Is this a low speed or full speed connection (USBFS does not
           * support high speed)
           */

          if ((hprt & USBFS_HPCS_PS_MASK) == USBFS_HPCS_PS_LS)
            {
              /* Set the Host Frame Interval Register for the 6KHz speed */

              usbhost_vtrace1(USBFS_VTRACE1_GINT_HPRT_LSDEV, 0);
              gd32_putreg(GD32_USBFS_HFT, 6000);

              /* Are we switching from FS to LS? */

              if ((hcfg & USBFS_HCTL_CLKSEL_MASK) !=
                  USBFS_HCTL_CLKSEL_LS6MHz)
                {
                  usbhost_vtrace1(USBFS_VTRACE1_GINT_HPRT_FSLSSW, 0);

                  /* Yes... configure for LS */

                  hcfg &= ~USBFS_HCTL_CLKSEL_MASK;
                  hcfg |= USBFS_HCTL_CLKSEL_LS6MHz;
                  gd32_putreg(GD32_USBFS_HCTL, hcfg);
          ...
          else /* if ((hprt & USBFS_HPCS_PS_MASK) == USBFS_HPCS_PS_FS) */
            {
              usbhost_vtrace1(USBFS_VTRACE1_GINT_HPRT_FSDEV, 0);
              gd32_putreg(GD32_USBFS_HFT, 48000);

              /* Are we switching from LS to FS? */

              if ((hcfg & USBFS_HCTL_CLKSEL_MASK) !=
                  USBFS_HCTL_CLKSEL_FS48MHz)
                {
                  usbhost_vtrace1(USBFS_VTRACE1_GINT_HPRT_LSFSSW, 0);

                  /* Yes... configure for FS */

                  hcfg &= ~USBFS_HCTL_CLKSEL_MASK;
                  hcfg |= USBFS_HCTL_CLKSEL_FS48MHz;
                  gd32_putreg(GD32_USBFS_HCTL, hcfg);
  ...
  gd32_putreg(GD32_USBFS_HPCS, newhprt);
}

/****************************************************************************
 * Name: gd32_gint_discisr
 *
 * Description:
 *   USBFS disconnect detected interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_discisr(struct gd32_usbhost_s *priv)
{
  ...
  gd32_gint_disconnected(priv);

  /* Clear the dicsonnect interrupt */

  gd32_putreg(GD32_USBFS_GINTF, USBFS_GINTF_DISCIF);
}

/****************************************************************************
 * Name: gd32_gint_ipxfrisr
 *
 * Description:
 *   USBFS incomplete periodic interrupt handler
 *
 ****************************************************************************/
...
static inline void gd32_gint_ipxfrisr(struct gd32_usbhost_s *priv)
{
  ...
  regval = gd32_getreg(GD32_USBFS_HCHCTL(0));
  regval |= (USBFS_HCHCTL_CDIS | USBFS_HCHCTL_CEN);
  gd32_putreg(GD32_USBFS_HCHCTL(0), regval);

  /* Clear the incomplete isochronous OUT interrupt */

  gd32_putreg(GD32_USBFS_GINTF, USBFS_GINTF_PXNCIF_ISOONCIF);
}
 ...
 *
 * Description:
 *   USBFS global interrupt handler
 *
 ****************************************************************************/

static int gd32_gint_isr(int irq, void *context, void *arg)
{
  /* At present, there is only support for a single USBFS host. Hence it is
  ...
  struct gd32_usbhost_s *priv = &g_usbhost;
  uint32_t pending;
  ...
  for (; ; )
    {
      ...
      pending  = gd32_getreg(GD32_USBFS_GINTF);
      pending &= gd32_getreg(GD32_USBFS_GINTEN);
...
#ifdef CONFIG_GD32E11X_USBFS_SOFINTR
      if ((pending & USBFS_GINTF_SOF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_SOF, 0);
          gd32_gint_sofisr(priv);
        }
#endif

      /* Handle the RxFIFO non-empty interrupt */

      if ((pending & USBFS_GINTF_RXFNEIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_RXFLVL, 0);
          gd32_gint_rxflvlisr(priv);
        }

      /* Handle the non-periodic TxFIFO empty interrupt */

      if ((pending & USBFS_GINTF_NPTXFEIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_NPTXFE, 0);
          gd32_gint_nptxfeisr(priv);
        }

      /* Handle the periodic TxFIFO empty interrupt */

      if ((pending & USBFS_GINTF_PTXFEIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_PTXFE, 0);
          gd32_gint_ptxfeisr(priv);
        }

      /* Handle the host channels interrupt */

      if ((pending & USBFS_GINTF_HCIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_HC, 0);
          gd32_gint_hcisr(priv);
        }

      /* Handle the host port interrupt */

      if ((pending & USBFS_GINTF_HPIF) != 0)
        {
          gd32_gint_hprtisr(priv);
        }

      /* Handle the disconnect detected interrupt */

      if ((pending & USBFS_GINTF_DISCIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_DISC, 0);
          gd32_gint_discisr(priv);
        }

      /* Handle the incomplete periodic transfer */

      if ((pending & USBFS_GINTF_PXNCIF_ISOONCIF) != 0)
        {
          usbhost_vtrace1(USBFS_VTRACE1_GINT_IPXFR, 0);
          gd32_gint_ipxfrisr(priv);
        }
    }
...
/****************************************************************************
 * Name: gd32_gint_enable and gd32_gint_disable
 *
 * Description:
 *   Respectively enable or disable the global USBFS interrupt.
 *
 * Input Parameters:
 *   None
 *
...
static void gd32_gint_enable(void)
{
  ...
  regval  = gd32_getreg(GD32_USBFS_GAHBCS);
  regval |= USBFS_GAHBCS_GINTEN;
  gd32_putreg(GD32_USBFS_GAHBCS, regval);
}
...
static inline void gd32_hostinit_enable(void)
{
  ...
  gd32_putreg(GD32_USBFS_GINTEN, 0);

  /* Clear any pending interrupts. */

  gd32_putreg(GD32_USBFS_GINTF, 0xffffffff);

  /* Clear any pending USB OTG Interrupts */

  gd32_putreg(GD32_USBFS_GOTGINTF, 0xffffffff);

  /* Clear any pending USB OTG interrupts */

  gd32_putreg(GD32_USBFS_GINTF, 0xbfffffff);

  /* Enable the host interrupts */

  /* Common interrupts:
   *
   *   USBFS_GINTF_WKUPIF     : Resume/remote wakeup detected interrupt
   *   USBFS_GINTF_SP  : USB suspend
   */

  regval = (USBFS_GINTF_WKUPIF | USBFS_GINTF_SP);

  /* If OTG were supported, we would need to enable the following as well:
   *
   *   USBFS_GINTF_OTGIF      : OTG interrupt
   *   USBFS_GINTF_SESIF      : Session request/new session detected
   *                            interrupt
   *   USBFS_GINTF_IDPSC  : Connector ID status change
   */

  /* Host-specific interrupts
   *
   *   USBFS_GINTF_SOF      : Start of frame
   *   USBFS_GINTF_RXFNEIF   : RxFIFO non-empty
   *   USBFS_GINTF_PXNCIF_ISOONCIF : Incomplete isochronous OUT transfer
   *   USBFS_GINTF_HPIF     : Host port interrupt
   *   USBFS_GINTF_HCIF       : Host channels interrupt
   *   USBFS_GINTF_DISCIF     : Disconnect detected interrupt
   */

#ifdef CONFIG_GD32E11X_USBFS_SOFINTR
  regval |= (USBFS_GINTF_SOF | USBFS_GINTF_RXFNEIF |
             USBFS_GINTF_PXNCIF_ISOONCIF | USBFS_GINTF_HPIF |
             USBFS_GINTF_HCIF | USBFS_GINTF_DISCIF);
#else
  regval |= (USBFS_GINTF_RXFNEIF | USBFS_GINTF_PXNCIF_ISOONCIF |
             USBFS_GINTF_HPIF | USBFS_GINTF_HCIF |
             USBFS_GINTF_DISCIF);
#endif
  gd32_putreg(GD32_USBFS_GINTEN, regval);
}

/****************************************************************************
 * Name: gd32_txfe_enable
 *
 * Description:
                                ...
                                int chidx)
{
  ...
  switch (chan->eptype)
    ...
    case USBFS_EPTYPE_CTRL: /* Non periodic transfer */
    case USBFS_EPTYPE_BULK:
      regval |= USBFS_GINTF_NPTXFEIF;
      break;

    case USBFS_EPTYPE_INTR: /* Periodic transfer */
    case USBFS_EPTYPE_ISOC:
      regval |= USBFS_GINTF_PTXFEIF;
      break;
    }
                                ...
                                struct usbhost_hubport_s *hport)
{
  ...
  regval = gd32_getreg(GD32_USBFS_HPCS);
  if ((regval & USBFS_HPCS_PS_MASK) == USBFS_HPCS_PS_LS)
    {
  ...
  ret = gd32_ctrlchan_alloc(priv, 0, 0, priv->rhport.hport.speed,
                               &priv->ep0);
  if (ret < 0)
    {
...
#endif
    {
      ret = gd32_rh_enumerate(priv, conn, hport);
      if (ret < 0)
        {
          return ret;
        }
...
static int gd32_ep0configure(struct usbhost_driver_s *drvr,
                                ...
                                uint8_t speed, uint16_t maxpacketsize)
{
  struct gd32_usbhost_s *priv = (struct gd32_usbhost_s *)drvr;
  struct gd32_ctrlinfo_s *ep0info =
    (struct gd32_ctrlinfo_s *)ep0;
  struct gd32_chan_s *chan;
  int ret;
  ...
  gd32_chan_configure(priv, ep0info->outndx);
  ...
  gd32_chan_configure(priv, ep0info->inndx);
                           ...
                           usbhost_ep_t *ep)
{
  ...
  if (epdesc->xfrtype == USBFS_EPTYPE_CTRL)
    {
      ret = gd32_ctrlep_alloc(priv, epdesc, ep);
    }
  else
    {
      ret = gd32_xfrep_alloc(priv, epdesc, ep);
    }
...
static int gd32_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep)
{
  struct gd32_usbhost_s *priv = (struct gd32_usbhost_s *)drvr;
  int ret;
   ...
   * GD32_MAX_TX_FIFOS.  Otherwise, the ep must be a pointer to an
   * allocated control endpoint structure.
   */

  if ((uintptr_t)ep < GD32_MAX_TX_FIFOS)
    {
      /* Halt the channel and mark the channel available */

      gd32_chan_free(priv, (int)ep);
    }
  else
    {
      ...
      struct gd32_ctrlinfo_s *ctrlep =
        (struct gd32_ctrlinfo_s *)ep;

      gd32_chan_free(priv, ctrlep->inndx);
      gd32_chan_free(priv, ctrlep->outndx);
                         ...
                         uint8_t **buffer, size_t *maxlen)
{
  ...
  /* There is no special memory requirement for the GD32E11x. */

  alloc = kmm_malloc(CONFIG_GD32E11X_USBFS_DESCSIZE);
  if (!alloc)
    {
      return -ENOMEM;
    }
  ...
  *buffer = alloc;
  *maxlen = CONFIG_GD32E11X_USBFS_DESCSIZE;
  return OK;
}
...
/****************************************************************************
 * Name: gd32_ctrlin and gd32_ctrlout
 *
 * Description:
                          ...
                          uint8_t *buffer)
{
  ...
  usbhost_vtrace2(USBFS_VTRACE2_CTRLIN, req->type, req->req);
  ...
  for (retries = 0; retries < GD32_RETRY_COUNT; retries++)
    {
      /* Send the SETUP request */

      ret = gd32_ctrl_sendsetup(priv, ep0info, req);
      if (ret < 0)
        {
          usbhost_trace1(USBFS_TRACE1_SENDSETUP, -ret);
          continue;
        }
      ...
      if (buflen > 0)
        {
          ret = gd32_ctrl_recvdata(priv, ep0info, buffer, buflen);
          if (ret < 0)
            {
              usbhost_trace1(USBFS_TRACE1_RECVDATA, -ret);
              continue;
            }
        }
      ...
      do
        {
          ...
          ret = gd32_ctrl_senddata(priv, ep0info, NULL, 0);
          if (ret == OK)
            {
          ...
          usbhost_trace1(USBFS_TRACE1_SENDDATA, ret < 0 ? -ret : ret);

          /* Get the elapsed time (in frames) */

          elapsed = clock_systime_ticks() - start;
        }
      while (elapsed < GD32_DATANAK_DELAY);
    }
                           ...
                           const uint8_t *buffer)
{
  ...
  DEBUGASSERT(priv != NULL && ep0info != NULL && req != NULL);
  usbhost_vtrace2(USBFS_VTRACE2_CTRLOUT, req->type, req->req);
  uinfo("type:%02x req:%02x value:%02x%02x index:%02x%02x len:%02x%02x\n",
        req->type, req->req, req->value[1], req->value[0],
        req->index[1], req->index[0], req->len[1], req->len[0]);
      ...
      do
        {
          ...
          if (buflen > 0)
            {
              ...
              priv->chan[ep0info->outndx].outdata1 = true;
              ret = gd32_ctrl_senddata(priv, ep0info,
                                       (uint8_t *)buffer, buflen);
              if (ret < 0)
                {
                  usbhost_trace1(USBFS_TRACE1_SENDDATA, -ret);
                }
            }
          ...
          if (ret == OK)
            {
              ret = gd32_ctrl_recvdata(priv, ep0info, NULL, 0);
              if (ret == OK)
                {
              ...
              usbhost_trace1(USBFS_TRACE1_RECVDATA, ret < 0 ? -ret : ret);
            }
                                ...
                                size_t buflen)
{
  ...
  DEBUGASSERT(priv && buffer && chidx < GD32_MAX_TX_FIFOS && buflen > 0);
  ...
  if (priv->chan[chidx].in)
    {
      nbytes = gd32_in_transfer(priv, chidx, buffer, buflen);
    }
  else
    {
      nbytes = gd32_out_transfer(priv, chidx, buffer, buflen);
    }
                          ...
                          usbhost_asynch_t callback, void *arg)
{
  ...
  if (priv->chan[chidx].in)
    {
      ret = gd32_in_asynch(priv, chidx, buffer, buflen, callback, arg);
    }
  else
    {
      ret = gd32_out_asynch(priv, chidx, buffer, buflen, callback, arg);
    }
...
static int gd32_cancel(struct usbhost_driver_s *drvr, usbhost_ep_t ep)
{
  struct gd32_usbhost_s *priv = (struct gd32_usbhost_s *)drvr;
  struct gd32_chan_s *chan;
  unsigned int chidx = (unsigned int)ep;
  ...
  DEBUGASSERT(priv && chidx < GD32_MAX_TX_FIFOS);
  chan = &priv->chan[chidx];
  ...
  gd32_chan_halt(priv, chidx, CHREASON_CANCELLED);
 ...
 *   USBFS_GOTGINTF), which indicates that the bus is stable again after
...
static void gd32_portreset(struct gd32_usbhost_s *priv)
{
  ...
  regval  = gd32_getreg(GD32_USBFS_HPCS);
  regval &= ~(USBFS_HPCS_PE | USBFS_HPCS_PCD | USBFS_HPCS_PEDC);
  regval |= USBFS_HPCS_PRST;
  gd32_putreg(GD32_USBFS_HPCS, regval);
...
static void gd32_flush_txfifos(uint32_t txfnum)
{
  ...
  gd32_putreg(GD32_USBFS_GRSTCTL, regval);
...
static void gd32_flush_rxfifo(void)
{
  ...
  gd32_putreg(GD32_USBFS_GRSTCTL, USBFS_GRSTCTL_RXFF);
...
static void gd32_vbusdrive(struct gd32_usbhost_s *priv, bool state)
{
  uint32_t regval;

  /* Enable/disable the external charge pump */

  gd32_usbhost_vbusdrive(0, state);
  ...
  if (((regval & USBFS_HPCS_PP) == 0) && state)
    {
      regval |= USBFS_HPCS_PP;
      gd32_putreg(GD32_USBFS_HPCS, regval);
    }
...
/****************************************************************************
 * Name: gd32_host_initialize
 *
 * Description:
 *   Initialize/re-initialize hardware for host mode operation.  At present,
 *   this function is called only from gd32_hw_initialize().  But if OTG
...
static void gd32_host_initialize(struct gd32_usbhost_s *priv)
{
  ...
  gd32_putreg(GD32_USBFS_PWRCLKCTL, 0);

  /* Initialize Host Configuration (HCFG) register */

  regval  = gd32_getreg(GD32_USBFS_HCTL);
  regval &= ~USBFS_HCTL_CLKSEL_MASK;
  regval |= USBFS_HCTL_CLKSEL_FS48MHz;
  gd32_putreg(GD32_USBFS_HCTL, regval);
  ...
  gd32_putreg(GD32_USBFS_GRFLEN, CONFIG_GD32E11X_USBFS_RXFIFO_SIZE);
  offset = CONFIG_GD32E11X_USBFS_RXFIFO_SIZE;

  /* Setup the host non-periodic Tx FIFO size (HNPTXFSIZ) */

  regval = (offset |
            (CONFIG_GD32E11X_USBFS_NPTXFIFO_SIZE <<
            USBFS_HNPTFLEN_HNPTXFD_SHIFT));
  gd32_putreg(GD32_USBFS_HNPTFLEN, regval);
  offset += CONFIG_GD32E11X_USBFS_NPTXFIFO_SIZE;

  /* Set up the host periodic Tx fifo size register (HPTXFSIZ) */

  regval = (offset |
            (CONFIG_GD32E11X_USBFS_PTXFIFO_SIZE <<
            USBFS_HPTFLEN_HPTXFD_SHIFT));
  gd32_putreg(GD32_USBFS_HPTFLEN, regval);
  ...
  for (i = 0; i < GD32_NHOST_CHANNELS; i++)
    {
      gd32_putreg(GD32_USBFS_HCHINTF(i), 0xffffffff);
      gd32_putreg(GD32_USBFS_HCHINTEN(i), 0);
    }
  ...
  gd32_vbusdrive(priv, true);

  /* Enable host interrupts */

  gd32_hostinit_enable();
}

/****************************************************************************
 * Name: gd32_sw_initialize
 *
 * Description:
...
static inline void gd32_sw_initialize(struct gd32_usbhost_s *priv)
{
  ...
  drvr                 = &priv->drvr;
  drvr->ep0configure   = gd32_ep0configure;
  drvr->epalloc        = gd32_epalloc;
  drvr->epfree         = gd32_epfree;
  drvr->alloc          = gd32_alloc;
  drvr->free           = gd32_free;
  drvr->ioalloc        = gd32_ioalloc;
  drvr->iofree         = gd32_iofree;
  drvr->ctrlin         = gd32_ctrlin;
  drvr->ctrlout        = gd32_ctrlout;
  drvr->transfer       = gd32_transfer;
#ifdef CONFIG_USBHOST_ASYNCH
  drvr->asynch         = gd32_asynch;
#endif
  drvr->cancel         = gd32_cancel;
#ifdef CONFIG_USBHOST_HUB
  drvr->connect        = gd32_connect;
#endif
  drvr->disconnect     = gd32_disconnect;
  ...
  memset(priv->chan, 0,
         GD32_MAX_TX_FIFOS * sizeof(struct gd32_chan_s));

  /* Initialize each channel */

  for (i = 0; i < GD32_MAX_TX_FIFOS; i++)
    {
      struct gd32_chan_s *chan = &priv->chan[i];
...
static inline int gd32_hw_initialize(struct gd32_usbhost_s *priv)
{
  ...
  /* GD32E11x GUSBCS has no PHYSEL bit (bits reserved) */

  /* Perform the core soft reset. */

  gd32_putreg(GD32_USBFS_GRSTCTL, USBFS_GRSTCTL_CSRST);
  ...
  regval |= USBFS_GCCFG_SOFOEN;
#endif
  gd32_putreg(GD32_USBFS_GCCFG, regval);
  up_mdelay(20);
  ...
  gd32_putreg(GD32_USBFS_GUSBCS, regval);
  up_mdelay(50);

  /* Initialize host mode and return success */

  gd32_host_initialize(priv);
  return OK;
}
...
struct usbhost_connection_s *gd32_usbfshost_initialize(int controller)
{
  /* At present, there is only support for a single USBFS host. Hence it is
   * pre-allocated as g_usbhost.  However, in most code, the private data
   * structure will be referenced using the 'priv' pointer (rather than the
  ...
  /* Make sure that interrupts from the USBFS core are disabled */

  gd32_gint_disable();

  /* Reset the state of the host driver */

  gd32_sw_initialize(priv);

  /* Alternate function pin configuration.  Here we assume that:
   *
   * 1. GPIOA, SYSCFG, and USBFS peripheral clocking have already been\
  ...
  /* Configure USBFS alternate function pins for DM, DP, ID, and SOF.
   *
  ...
  /* Initialize the USBFS core */

  gd32_hw_initialize(priv);

  /* Attach USB host controller interrupt handler */

  if (irq_attach(GD32_IRQ_USBFS, gd32_gint_isr, NULL) != 0)
    {
      usbhost_trace1(USBFS_TRACE1_IRQATTACH, 0);
      return NULL;
    }

  /* Enable USBFS global interrupts */
...
#endif /* CONFIG_USBHOST && CONFIG_GD32E11X_USBFS */